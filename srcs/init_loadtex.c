...
/*   Updated: 2024/08/22 10:21:35 by mburakow         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

void validate_and_load_texture(t_data *data, const char *path, int index, char *error_msg, int error_code)
{
	...
	if (data->txtrs[index]->width > 4096 || data->txtrs[index]->height > 4096)
		free_all_and_quit(data, "image dimensions should be less than 4096 pixels", error_code + 2);
}
...
void load_nsew_textures(t_data *data)
{
	for (int i = 0; i < 4; i++)
	{
		validate_and_load_texture(data, data->nsew_path[i], i, (char *)"can't find texture file", 76 + i * 3);
	}
}

void load_specific_texture(t_data *data, const char *path, int index, char *error_msg, int error_code)
{
