...
/*   Updated: 2024/07/09 15:07:30 by mburakow         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_enemy_onto_canvas(t_data *data, t_enemy * enemy, int dest_x, int dest_y) 
{
    t_point     pt;
    mlx_image_t *dest;
    mlx_image_t *src;
...
void    draw_enemy(t_data *data, t_enemy *enemy, uint32_t screen_x)
{
    ...
    ft_memcpy(data->drawframe->pixels, current->pixels, current->width * current->height * sizeof(uint32_t));
...
void	hook_enemies(t_data *data)
{
    ...
    while (data->enemies[++i] != NULL)
    {
        if (data->enemies[i]->visible)
        {
            ...
            //data->enemies[i]->start_ang = data->enemies[i]->rel_angle - enemy_angle_width;
            //data->enemies[i]->end_ang = data->enemies[i]->rel_angle + enemy_angle_width;
            // Convert angles to corresponding rays
            //data->enemies[i]->start_posx = (int)((data->enemies[i]->start_ang + rad(FOV / 2)) / rad(FOV) * data->width);
            //data->enemies[i]->end_posx = (int)((data->enemies[i]->end_ang + rad(FOV / 2)) / rad(FOV) * data->width);
